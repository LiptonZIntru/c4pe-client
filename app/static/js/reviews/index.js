            ...
            reviews.forEach(review => {
                if(review.user.id == user_id)
                {
                    ...
                    content += '<div class="card m-2 mt-1 mb-3">' +
                        ...
                        '<small class="float-right pt-2" id="staticRating">' +
                        rating +
                        '</small>' +
                        ...
                        '</small>' +
                        '<div class="row">' +
                        '<p class="text-nowrap mb-2 pt-1 mr-2">User ' +
                        user_icon +
                        '<a href="/users/' + review.user.id + '" class="text-dark"><b>' + review.user.username + '</b></a>' +
                        ' says:' +
                        '</p>' +
                        '<button type="button" class="btn btn-outline-secondary py-1 px-2 mb-2" onclick="editReview(' +
                        review.rating + ');" id="editReviewButton">' +
                        'Edit </button> <button type="submit" class="btn btn-outline-secondary py-1 px-2 mb-2" form="editReview" onclick="editReview(0);" id="submitReviewButton" hidden>' +
                        'Save' +
                        '</button>' +
                        '</div>' +
                        '<label for="editReviewTextArea" hidden></label>' +
                        '<form id="editReview" method="post" action="/places/' + place_id + '/reviews/' + review.id + '/edit/" hidden>' +
                        ...
                        '<textarea onchange="updateText(\'editReviewText\', this.id);" name="editReviewTextArea" class="card p-2 pl-3 text-justify text-md-smaller disabled w-100" id="editReviewTextArea" rows="3" style="min-height: 2.7em; max-height: 27em;">' +
